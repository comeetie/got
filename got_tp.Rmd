...
Servez vous de la table scenes pour calculer le nombre de personnages morts lors de la première saison.
```
...
Construisez à partir de la data.frame précédente une matrice contenant uniquement les variables numériques. Filtrez la pour ne conservez que les lignes dont la somme est supérieur à 3600. Normalisez là pour que les sommes en lignes soient égales à 1. Donnez le nom de chaqué personnage conserver à la ligne correspondante dans la matrice avec la fonction `rownames`.
```
...
A l'aide de la fonction `dist` calculer la distance de manhatan entre chque lignes de la matrice précédente. Réalisez ensuite un clustering hierarhique avec cette matrice de distance et affichez le résultat. Vous devriez obtennir une figure similaire à la figure suivante : 
```
...
gg=kingdoms %>% left_join(pop)

ggplot(gg)+geom_sf(aes(fill=nbcastles/population*10000))+geom_sf_text(aes(label=ClaimedBy)) +coord_sf(expand = 0,ndiscr = 0)+theme(panel.background = element_rect(fill = colriver,color=NA),
        text = element_text(family="Palatino",face = "bold",size = 14),
        legend.key = element_rect(fill="#ffffff"),
        ) +scale_color_distiller("Chateaux / 10000 hab",pallette="RdOr")+
  labs(title ="Chateaux / habitants",caption = "@comeetie, 2020",x="",y="")
