...
Dans ce TP nous allons pratiquer un peu de R, en explorant les données de la série Game of thrones avec la librairie `dplyr` et les visualiser. Les données utilisées ont été collectées par Jeffrey Lancaster et sont issues de [ce projet](https://github.com/jeffreylancaster/game-of-thrones).

https://hitek.fr/actualite/distance-temporalite-game-of-thrones-saison-7_13898
...
Importer les fichiers  characters.csv, episodes.csv, scenes.csv et appearances.csv du répertoire data et les stocker dans des variables de mêmes nom.
```


```{block, opts.label = "clues"}
Utilisez `readr::read_csv()` de préfrénce de la libraririe readr pour une meilleure reconnaissance automatique des types des variables.
```
...
Regardez la taille de la data.frame appearences, observez les variables communes aux tables scenes et appearences. Utilisez les fonctions `str` et `summary` sur les différentes tables pour bien comprendre leurs structures et leurs relations.
```


```{block, opts.label = "clues"}
Utiliser `nrow, ncol` ou `dim` pour connaitres les dimensions des data.frame. Utiliser la fonction `names` pour connaitres le noms des colones et l'opérateur `%in%` ou la fonction setdiff par exemple pour trouver les variables communes.
```
...
Trouvez le nombre de scènes se passant à Port Real.
```
...
Construisez une data.frame contenant pour chaque épisode son titre, la saison, la durée le la scène la plus longue, le nombre de scènes et le nombre de mort.
...
```{block, box.title = "Scatter-plots", box.body = list(fill = "white"), box.icon = "fa-star"}
Enfin, utiliser la couleur et la taille des points pour encoder des informations sur les saisons et le nombre de morts et finalisez le graphique qui pourrait ressembler à cette version en paramétrant les échelles et en rajoutant quelques labels.
```
...
Quelle famille possède le plus de chateau ?
Mais cette fois si utilisez la fonction la fonction `st_overlaps` .
```
...
```{block, box.title = "Intresections, aggrégations", box.body = list(fill = "white"), box.icon = "fa-star"}
...
# Exercise 6 : cartographie avec les packages sf, ggplot et cartography
