...
```{block, opts.label = "clues"}
Use `nrow, ncol` or `dim` to know the dimensions of the data.frame. Use the `names` function to know the names of the columns and the `%in%` operator or the `setdiff` function for example to find common variables.
```
